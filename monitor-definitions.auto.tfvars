...
"id014" = { # Needs to be unique
    ...
    {{#is_no_data}}This monitor is missing data and can no longer monitor the usecase annotator service. It's possible metrics are no longer being submitted. Check the OpenShift deployment still exists and is in good health. Make sure the metric and tags this monitor checks still matches the deployment. If there is no explanation for missing data, it could be the metrics are not being submitted to DataDog and you should reach out to the Voltron team.{{/is_no_data}}
    {{#is_no_data_recovery}}This monitor is no longer missing data.{{/is_no_data_recovery}}
    Playbook: https://secureworks.atlassian.net/wiki/spaces/VOLPLAY/pages/468258947164/Queue+size+-+Usecase+Annotator
...
####### Usecase search service prod pod count monitor START #######

"id015" = { # Needs to be unique
    monitor_name    = "Usecase search service prod pod count < 2"
    monitor_query   = "max(last_15m):avg:kubernetes_state.deployment.replicas_available{env:prod,kube_namespace:usecase,service:search-service} < 2"
    alert_message   = <<EOF
    {{#is_alert}}
    prod search service has less than 2 pods running. Check the deployment in OpenShift. 
    {{/is_alert}} 
    {{#is_alert_recovery}}
    prod search service capacity is recovered. 
    {{/is_alert_recovery}} 
    {{#is_no_data}}This monitor is missing data and can no longer monitor the usecase search service. It's possible metrics are no longer being submitted. Check the OpenShift deployment still exists and is in good health. Make sure the metric and tags this monitor checks still matches the deployment. If there is no explanation for missing data, it could be the metrics are not being submitted to DataDog and you should reach out to the Voltron team.{{/is_no_data}}
    {{#is_no_data_recovery}}This monitor is no longer missing data.{{/is_no_data_recovery}}
    Playbook: https://secureworks.atlassian.net/wiki/spaces/VOLPLAY/pages/467878281271/
    ...
    tag_list = [
        ...
        "playbook:yes",
        "openshift-deployment-name:search-service",
        "openshift-namespace:usecase",
        "recovery-timeout-minutes:25"]
  },
####### Usecase search service prod pod count monitor END #######

####### Usecase event worker prod pod count monitor START #######

"id016" = { # Needs to be unique
    monitor_name    = "Usecase event worker prod pod count < 1"
    monitor_query   = "max(last_15m):avg:kubernetes_state.deployment.replicas_available{env:prod,kube_namespace:usecase,service:mod-eventworker} < 1"
    alert_message   = <<EOF
    {{#is_alert}}
    prod event worker has less than 1 pod running. Check the deployment in OpenShift. 
    {{/is_alert}} 
    {{#is_alert_recovery}}
    prod event worker capacity is recovered. 
    {{/is_alert_recovery}} 
    {{#is_no_data}}This monitor is missing data and can no longer monitor the usecase event worker. It's possible metrics are no longer being submitted. Check the OpenShift deployment still exists and is in good health. Make sure the metric and tags this monitor checks still matches the deployment. If there is no explanation for missing data, it could be the metrics are not being submitted to DataDog and you should reach out to the Voltron team.{{/is_no_data}}
    {{#is_no_data_recovery}}This monitor is no longer missing data.{{/is_no_data_recovery}}
    Playbook: https://secureworks.atlassian.net/wiki/spaces/VOLPLAY/pages/467878281271/
...
####### Usecase Harvester prod pod count monitor START #######

"id018" = { # Needs to be unique
    monitor_name    = "Usecase Harvester prod pod count < 1"
    monitor_query   = "max(last_15m):avg:kubernetes_state.deployment.replicas_available{env:prod,kube_namespace:usecase,service:mod-harvester} < 1"
    alert_message   = <<EOF
    {{#is_alert}}
    prod Harvester has less than 1 pod running. Check the deployment in OpenShift. 
    {{/is_alert}} 
    {{#is_alert_recovery}}
    prod Harvester capacity is recovered. 
    {{/is_alert_recovery}} 
    {{#is_no_data}}This monitor is missing data and can no longer monitor the usecase harvester. It's possible metrics are no longer being submitted. Check the OpenShift deployment still exists and is in good health. Make sure the metric and tags this monitor checks still matches the deployment. If there is no explanation for missing data, it could be the metrics are not being submitted to DataDog and you should reach out to the Voltron team.{{/is_no_data}}
    {{#is_no_data_recovery}}This monitor is no longer missing data.{{/is_no_data_recovery}}
    Playbook: https://secureworks.atlassian.net/wiki/spaces/VOLPLAY/pages/467878281271/
...
####### Usecase Harvester prod pod count monitor END #######

####### Usecase Devtypemapper prod pod count monitor START #######

"id019" = { # Needs to be unique
    monitor_name    = "Usecase Devtypemapper prod pod count < 1"
    monitor_query   = "max(last_15m):avg:kubernetes_state.deployment.replicas_available{env:prod,kube_namespace:usecase,service:mod-devtypemapper} < 1"
    alert_message   = <<EOF
    {{#is_alert}}
    prod Devtypemapper has less than 1 pod running. Check the deployment in OpenShift. 
    {{/is_alert}} 
    {{#is_alert_recovery}}
    prod Devtypemapper capacity is recovered. 
    {{/is_alert_recovery}} 
    {{#is_no_data}}This monitor is missing data and can no longer monitor the usecase devtypemapper. It's possible metrics are no longer being submitted. Check the OpenShift deployment still exists and is in good health. Make sure the metric and tags this monitor checks still matches the deployment. If there is no explanation for missing data, it could be the metrics are not being submitted to DataDog and you should reach out to the Voltron team.{{/is_no_data}}
    {{#is_no_data_recovery}}This monitor is no longer missing data.{{/is_no_data_recovery}}
    Playbook: https://secureworks.atlassian.net/wiki/spaces/VOLPLAY/pages/467878281271/
...
####### Usecase Devtypemapper prod pod count monitor END #######
