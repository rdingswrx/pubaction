...
"id007" = { # Needs to be unique
    monitor_name    = "Netacuity prod pod count < 1"
    monitor_query   = "max(last_15m):avg:kubernetes_state.deployment.replicas_available{env:prod,kube_namespace:foresee,service:netacuity-service} < 1"
    alert_message   = <<EOF
    {{#is_alert}}
    prod netacuity-service has no pods running. Check the deployment in OpenShift.  
    {{/is_alert}} 
    {{#is_alert_recovery}}
    prod netacuity-service capacity is recovered.
    {{/is_alert_recovery}} 
    ...
    tag_list = [
        ...
        "notify:oncallbot", 
        "env:prod", 
        "service:netacuity-service", 
        "playbook:yes",
        "openshift-deployment-name:netacuity-service-deployment",
        "openshift-namespace:foresee",
        "recovery-timeout-minutes:25"]
  },
####### Netacuity prod pod count monitor END #######
